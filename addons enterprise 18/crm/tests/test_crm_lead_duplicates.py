...
class TestCRMLead(TestCrmCommon):
    ...
    def setUpClass(cls):
        ...
        cls.emails_provider_generic = {
            ...
            ('fp@netprocess.ma', 'fp@netprocess.ma'),
            ('fp.alias@mail.netprocess.ma', 'fp.alias@mail.netprocess.ma'),
        }
        cls.emails_provider_company = {
            ('robert.poilvert@mycompany.com', 'mycompany.com'),
            ('fp@subdomain.netprocess.ma', 'subdomain.netprocess.ma'),
        }
        ...
        cls.lead_generic_email_dupes = cls.env['crm.lead'].create([
            ...
            {
                'email_from': '"Fabulous Fab" <fp@netprocess.ma>',
                'name': 'Dupe1 of fp@netprocess.ma (same email)',
                'type': 'lead',
            },
            {
                'email_from': 'FP@netprocess.ma',
                'name': 'Dupe2 of fp@netprocess.ma (same email)',
                'type': 'lead',
            },
            # phone_sanitized based
            {
                'email_from': 'not.fp@not.netprocess.ma',
                'name': 'Dupe3 of fp@netprocess.ma (same phone sanitized)',
                'phone': '+1 202 555 0123',
                'type': 'lead',
            },
            {
                ...
                'name': 'Dupe4 of fp@netprocess.ma (same phone sanitized)',
                'type': 'lead',
            },
            # same commercial entity
            {
                'name': 'Dupe5 of fp@netprocess.ma (same commercial entity)',
                'partner_id': cls.test_partners[1].id,
            },
            {
                'name': 'Dupe6 of fp@netprocess.ma (same commercial entity)',
        ...
        cls.lead_generic_email_notdupes = cls.env['crm.lead'].create([
            ...
            {
                'email_from': 'not.fp@netprocess.ma',
                'name': 'NotADupe1',
                'type': 'lead',
            },
        ])
    ...
    def test_assert_initial_values(self):
        ...
        self.assertEqual(lead_generic.email_domain_criterion, 'fp@netprocess.ma')
        self.assertEqual(lead_generic.email_normalized, 'fp@netprocess.ma')
