...
class TestWebPushNotification(SMSCommon):
    ...
    def setUpClass(cls):
        ...
        cls.env['mail.partner.device'].sudo().create([{
            'endpoint': 'https://test.netprocess.ma/webpush/user1',
        ...
        cls.env['mail.partner.device'].sudo().create([{
            'endpoint': 'https://test.netprocess.ma/webpush/user2',
            'expiration_time': None,
            'keys': json.dumps({
                'p256dh': 'BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A',
                'auth': 'DJFdtAgZwrT6yYkUMgUqow'
    ...
    def test_push_notifications(self, push_to_end_point):
        ...
        self.assertEqual(push_to_end_point.call_args.kwargs['device']['endpoint'], 'https://test.netprocess.ma/webpush/user2')
    ...
    def test_push_notifications_cron(self, push_to_end_point):
        ...
        for index in range(10, 14):
            self.env['mail.partner.device'].sudo().create([{
                'endpoint': 'https://test.netprocess.ma/webpush/user%d' % index,
                'expiration_time': None,
                'keys': json.dumps({
                    'p256dh': 'BGbhnoP_91U7oR59BaaSx0JnDv2oEooYnJRV2AbY5TBeKGCRCf0HcIJ9bOKchUCDH4cHYWo9SYDz3U-8vSxPL_A',
                    'auth': 'DJFdtAgZwrT6yYkUMgUqow'
    ...
    def test_push_notifications_encryption_simple(self, post):
        ...
        self.assertEqual(post.call_args.args[0], 'https://test.netprocess.ma/webpush/user2')
    ...
    def test_push_notifications_device_unreachable(self, post):
        ...
        notification_count = self.env['mail.partner.device'].search_count([('endpoint', '=', 'https://test.netprocess.ma/webpush/user2')])
