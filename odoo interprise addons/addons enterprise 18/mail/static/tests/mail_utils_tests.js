...
QUnit.test("addLink: utility function and special entities", function (assert) {
    const testInputs = {
        ...
        "https://netprocess.ma/%5B%5D": `<a target="_blank" rel="noreferrer noopener" href="https://netprocess.ma/%5B%5D">https://netprocess.ma/[]</a>`,
    };
...
QUnit.test("url", async () => {
    ...
    const messageBody = "https://netprocess.ma?test=~^|`{}[]#";
...
QUnit.test("url with comma at the end", async () => {
    ...
    const { openDiscuss } = await start();
    await openDiscuss(channelId);
    const messageBody = "Go to https://netprocess.ma, it's great!";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button:enabled", { text: "Send" });
    await contains(".o-mail-Message a", { text: "https://netprocess.ma" });
...
QUnit.test("url with number in subdomain", async () => {
    ...
    const { openDiscuss } = await start();
    await openDiscuss(channelId);
    const messageBody = "https://www.45017478-master-all.runbot134.netprocess.ma/web";
    await insertText(".o-mail-Composer-input", messageBody);
    await click("button:enabled", { text: "Send" });
    await contains(".o-mail-Message a", {
        text: "https://www.45017478-master-all.runbot134.netprocess.ma/web",
    });
});
