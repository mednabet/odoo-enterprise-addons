...
QUnit.test("click a web link", async (assert) => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: { content: "[Odoo](https://netprocess.ma)" } },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    assert.strictEqual(urlRepresentation(cell.link, model.getters), "https://netprocess.ma");
    openLink(cell.link, env);
    assert.verifySteps(["https://netprocess.ma"]);
});
